function process_NE511_VT_XmlFeed_to_Discord() {
  NE511_VT_incidentData_XmlFeed_to_Discord();
  NE511_VT_laneClosureData_XmlFeed_to_Discord();
}

//Pass anything with the following key words.
var postPatternVT = /\b(closed|closure|blocked|blocking|flooded|tree in|water over|shut\s+down|all\s+lanes|both\s+lanes|across\s+the\s+(road|street))\b/i

// Then filter out anything that is Lane spacific.
var exclusionPatternVT = /(?:\b(Lane\s+(1|2|3|4|one|two|three|four))|((Right|Left|Center|ORT|EZ\s+PASS|EZPASS|cash|shoulder|median|1|one|2|two|3|three|Right\s+Turn|Left\s+Turn)\s+lane(s)?|(shoulder|median))\s+(is\s+|are\s+)?(currently\s+)?(closed|closure|blocked)|Closed\s+temporarily|\b(one|1|two|2)\s+(northbound|southbound|eastbound|westbound|NB|SB|EB|WB)\s+lane\s+(is\s+|are\s+)?(currently\s+)?(closed|blocked)|closed\s+to\s+(one|1|two|2|Marine\s+traffic)\s+lane|down\s+to\s+(one|1|two|2)\s+Lane|closed\s+to\s+Marine\s+traffic|(?:a|single|multiple|rolling)?\s*lane\s+closures?|rolling\s+closure|rolling\s+roadblock)\b/i;

function NE511_VT_incidentData_XmlFeed_to_Discord() {
  // Initialize counters
  var postedCount = 0;
  var skippedCount = 0;

  // URL of the XML source
  var url = 'https://nec-por.ne-compass.com/XmlDataPortalV2/api/c2c?networks=Vermont&dataTypes=incidentData'; // New URL updated 3-25-2024
  var vt_webohok = 'ENTER_WEBHOOK_URL_HERE';
  ...
  var lastRunTime = PropertiesService.getScriptProperties().getProperty('NE_511_VT_incidentData_lastRunTime');
  ...
  incidents.forEach(function (incident) {
    ...
    if ((createdTime > lastRunTime)) {
      ...
      if (postPatternVT.test(description)) {
        // Now check for exclusion criteria
        if (exclusionPatternVT.test(description)) {
        ...
        var payload = JSON.stringify({
          ...
          embeds: [{
            ...
            url: `https://newengland511.org/map#Incidents-Vermont511Incident--${id}`,
            author: {
        ...
        UrlFetchApp.fetch(vt_webohok, params);
  ...
  PropertiesService.getScriptProperties().setProperty('NE_511_VT_incidentData_lastRunTime', currentTime.toISOString());
}

function NE511_VT_laneClosureData_XmlFeed_to_Discord() {
  // Initialize counters
  var postedCount = 0;
  var skippedCount = 0;

  // URL of the XML source
  var url = 'https://nec-por.ne-compass.com/XmlDataPortalV2/api/c2c?networks=Vermont&dataTypes=laneClosureData'; // New URL updated 3-25-2024
  var vt_webohok = 'https://discord.com/api/webhooks/1110535074895646780/ttXP9McDqPNkvq86FgaCL4NVaKRHgJ5eWAFxANy6vB45cvGUfz3yZF01M-7ZV8MBssKn'; // NER --> VT --> VT Twitter --> NE 511 Webhook
  //var vt_webohok = 'https://discord.com/api/webhooks/1131294055955959918/9oM-xAliEBHqvOEmvCcffg0rQgq4V60umDfey-FPaZ5gkGtEp8Mgh6jy1eX0KN2EbAt8';    //JS55CT-->mass-vt-511-test
  ...
  // Retrieve the last run time of the script from properties or use 0 if it's the first time
  var lastRunTime = PropertiesService.getScriptProperties().getProperty('NE_511_VT_laneClosureData_lastRunTime');
  lastRunTime = lastRunTime ? new Date(lastRunTime) : new Date(0);
      ...
      if (postPatternVT.test(description)) {
        ...
        var payload = JSON.stringify({
          ...
          embeds: [{
            ...
            url: `https://newengland511.org/map#ConstructionClosures-Vermont511Closure--${id}`,
            author: {
              name: 'New England 511',
              url: 'https://newengland511.org/',
            },
  ...
  // Store the current time as the last run time
  PropertiesService.getScriptProperties().setProperty('NE_511_VT_laneClosureData_lastRunTime', currentTime.toISOString());
}
